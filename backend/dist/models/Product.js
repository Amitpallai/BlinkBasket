...
productSchema.pre("findOneAndUpdate", async function (next) {
    ...
    if (doc) {
        ...
        const newOfferPrice = update.offerPrice !== undefined ? update.offerPrice : doc.offerPrice;
        // Validate
        if (newOfferPrice > newPrice) {
            next(new Error(`Offer price (${newOfferPrice}) must be less than or equal to regular price (${newPrice})`));
            return;
        }
    }
    next();
